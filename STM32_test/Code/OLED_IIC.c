...
#define OLED_SCL_OUT_HIGH()		HAL_GPIO_WritePin(SCL_GPIO_Port,SCL_Pin,GPIO_PIN_SET)
#define OLED_SCL_OUT_LOW()      HAL_GPIO_WritePin(SCL_GPIO_Port,SCL_Pin,GPIO_PIN_RESET)
#define OLED_SDA_OUT_HIGH()     HAL_GPIO_WritePin(SDA_GPIO_Port,SDA_Pin,GPIO_PIN_SET)
#define OLED_SDA_OUT_LOW()		HAL_GPIO_WritePin(SDA_GPIO_Port,SDA_Pin,GPIO_PIN_RESET)
#define OLED_SDA_Write(XX)   	HAL_GPIO_WritePin(SDA_GPIO_Port,SDA_Pin,(XX?GPIO_PIN_SET:GPIO_PIN_RESET))
...
void OLED_DrawPoint(uint8_t row,uint8_t col,uint8_t mode)
{
	...
	m = row%8;
	n = 1 << m;
	if(mode == 1)	OLED_GRAM[col][i] |= n;
	else
	{
		OLED_GRAM[col][i]  = ~OLED_GRAM[col][i];
		OLED_GRAM[col][i] |= n;
		OLED_GRAM[col][i]  = ~OLED_GRAM[col][i];
	}
}
...
{
	char Data[16];             //创建目标数组，用来存放转换后的字符数据 
    sprintf(Data,"%.2f",FloatNum);    //保留小数点后3位小数，打印到Data数组中
	unsigned char length = strlen(Data);
	for(uint8_t i = length;i < 16 - y;i++)
		Data[i] = ' ';
	Data[16 - y] = '\0';
