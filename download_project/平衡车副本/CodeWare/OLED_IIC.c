...
#define OLED_SDA_Write(XX)   	HAL_GPIO_WritePin(OLED_SDA_GPIO_Port,OLED_SDA_Pin,(XX?GPIO_PIN_SET:GPIO_PIN_RESET))
...
{
	uint8_t temp;  // 缓存区保存编码字节
	uint8_t row_basic = (Line - 1) * 16,col_basic = (Column - 1) * 8;
	uint8_t row = row_basic,col = col_basic;
	for(uint8_t i = 0;i < 16;i++)
	{
		temp = OLED_F8x16[Char - ' '][i];  // 获取字符对应的编码字节
		
		for(uint8_t m = 0;m < 8;m++)
		{
			if(temp&0x01)
				OLED_DrawPoint(row,col,1);
			else 
				OLED_DrawPoint(row,col,0);
			temp>>=1;
			row++;
		}
		col++;
...
{
	char Data[14];             //创建目标数组，用来存放转换后的字符数据 
