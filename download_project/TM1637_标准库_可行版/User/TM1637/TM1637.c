...
// 最高位设置为1时显示 数码管上的":" 符号
unsigned char disp_num[] = {0x3F, 0x06 | 0x80, 0x5B, 0x4F};			//存放6个数码管要显示的内容
...
//写显示寄存器 写SRAM数据地址自动加1模式
void TM1637_Display_INC( void )
{
    unsigned char i;
    TM1637_Start();
    TM1637_WriteByte( 0x40 );		//写数据到显示寄存器 40H 地址自动加1 模式,44H 固定地址模式,该部分采用自加1模式
    TM1637_Ack();
    TM1637_Stop();
    TM1637_Start();
...
void TM1637_Display_NoINC( unsigned char add, unsigned char value )
{
    ...
    TM1637_Start();
    TM1637_WriteByte( 0x44 );		//写数据到显示寄存器 40H 地址自动加1 模式,44H 固定地址模式,该部分采用固定地址模式
    TM1637_Ack();
    TM1637_Stop();
...
// 开显示，并且设置亮度level : 0---7 （0 为灭，7 为最大）
