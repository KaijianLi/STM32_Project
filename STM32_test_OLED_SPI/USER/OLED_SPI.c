...
void OLED_ScrollDisplay_String(int y,char *str,int length,int space,int mode)
{
	int displayed_num = 0,flag = 0;
	while(1)
	{
		if(flag == 0)
		{
			if(y > 63) y = 0;
			OLED_ShowChar(128,y,*(str + displayed_num),16,mode);
			displayed_num++;
		}
		flag++;
		if(flag == 16) flag = 0;
		if(displayed_num == length)
		{
			displayed_num = 0;
			y += 16;
			for(int r = 0;r < 16 * space;r++)      //显示间隔
			{
				int n = 0,i = 0;
				for(i = 1;i < 144;i++)
				{
		...
		for(int i = 1;i < 144;i++)   //实现左移，该循环每循环一次，OLED屏幕上的现有亮点左移一次
		{
			int n = 0;
			for(n=0;n<8;n++)
				OLED_GRAM[i-1][n] = OLED_GRAM[i][n];
...
//space 每一遍显示的间隔
//mode:0,反色显示;1,正常显示
void OLED_ScrollDisplay_Chinese(int y,int num,int space,int mode)
{
	int displayed_num = 0,flag = 0;
	while(1)
	{
		...
		{
			if(y > 63) y = 0;
			OLED_ShowChinese(128,y,displayed_num,16,mode); //写入一个汉字保存在OLED_GRAM[][]数组中
			displayed_num++;
		}
		
		flag++;
		if(flag == 16)	flag=0;	// 一个汉字已经移位完成，标志位置零，开始显示下一个汉字
		
		for(int i = 1;i < 144;i++)   //实现左移，该循环每完整运行一次，OLED屏幕上的现有亮点左移一个单位
		{
			int n = 0;
			for(n=0;n<8;n++)
				OLED_GRAM[i-1][n] = OLED_GRAM[i][n];
		...
		if(displayed_num == num)	// 一组汉字已经显示完全，开始显示间隔
		{
			y += 16;	// 换行
			for(int r = 0;r < 16 * space;r++)   //显示间隔
			{
			...
			displayed_num = 0;	// 间隔符显示完全，计数值清零，重新开始显示汉字
       }
		OLED_Refresh();
	}
}
